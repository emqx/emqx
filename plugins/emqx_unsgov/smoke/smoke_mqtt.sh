...
check_plugin_api_ready() {
    ...
    if [[ "$code" == "404" ]] && grep -Fq "Plugin API Not Found" <<<"$body"; then
        echo "[smoke][mqtt] plugin API is not registered at $PLUGIN_BASE" >&2
        echo "[smoke][mqtt] ensure plugin is installed and started before running this script." >&2
        echo "[smoke][mqtt] tip: run ./plugins/emqx_unsgov/smoke/smoke_full.sh" >&2
    else
