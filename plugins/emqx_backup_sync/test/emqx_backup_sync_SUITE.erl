...
t_sync_once_passes_primary_ssl_options_to_httpc(Config) ->
    ...
    ok = file:write_file(Cacertfile, cert_file("cacert.pem")),
    BackupName = <<"emqx-export-2026-06-02.tar.gz">>,
...
create_primary_http_connector_action(PrimaryNode) ->
    ?ON(PrimaryNode, begin
        ...
        {ok, _Connector} = emqx_connector:create(
            ?global_ns, <<"http">>, ?INTEGRATION_HTTP_CONNECTOR_NAME, ConnectorConfig
        ),
        {ok, _Action} = emqx_bridge_v2:create(
            ?global_ns, actions, <<"http">>, ?INTEGRATION_HTTP_ACTION_NAME, ActionConfig
        ),
        {ok, #{data_dir => emqx:data_dir(), certs => Certs}}
    end).
...
has_primary_rule(PrimaryNode) ->
    ?ON(PrimaryNode, begin
        case emqx_rule_engine:get_rule(?global_ns, ?INTEGRATION_RULE_ID) of
            {ok, _Rule} -> true;
            _ -> false
        end
    end).
...
trigger_sync_and_wait_finished(Node) ->
    _ = ?ON(Node, begin
        whereis(emqx_backup_sync) ! sync,
        emqx_backup_sync:status()
    end),
    wait_until(
        fun() ->
            case ?ON(Node, emqx_backup_sync:status()) of
                #{worker := idle, health := ok} -> true;
                _ -> false
            end
        end,
        #{action => wait_backup_sync_finished, node => Node, status => sync_status(Node)}
    ).

sync_status(Node) ->
    ?ON(Node, emqx_backup_sync:status()).
...
make_upload_error_backup(_Config, _BackupName) ->
...
assert_real_sync_finished(SelectedNode, RootKeys, TableSets) ->
    ?check_trace(
        begin
            trigger_sync_and_wait_finished(SelectedNode)
        end,
        fun(Trace) ->
            assert_success_trace(Trace, SelectedNode, RootKeys, TableSets)
        end
