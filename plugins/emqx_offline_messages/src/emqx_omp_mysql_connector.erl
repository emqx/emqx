...
-module(emqx_omp_mysql_connector).
...
on_start(
    ...
    ?SLOG(info, #{
        msg => "starting_omp_mysql_connector",
        connector => InstId,
    ...
    case emqx_resource_pool:start(InstId, ?MODULE, Options ++ PrepareOpt ++ SslOpts) of
        ...
        {error, Reason} ->
            ?tp(
                omp_mysql_connector_start_failed,
                #{error => Reason}
            ),
            {error, Reason}
    end.
...
on_stop(InstId, _State) ->
    ?SLOG(info, #{
        msg => "stopping_omp_mysql_connector",
        connector => InstId
    }),
...
do_on_query(Fun, LogInfo, #{pool_name := PoolName} = _State) ->
    ?TRACE("QUERY", "omp_mysql_connector_do_on_query", LogInfo),
    Worker = ecpool:get_client(PoolName),
    case ecpool_worker:client(Worker) of
        {ok, Conn} ->
            try Fun(Conn) of
                {error, Reason} ->
                    ?tp(warning, "omp_mysql_connector_query_failed", LogInfo#{reason => Reason}),
                    {error, Reason};
                Result ->
                    Result
            catch
                Class:Reason ->
                    ?tp(error, "omp_mysql_connector_query_exception", LogInfo#{
                        class => Class, reason => Reason
                    }),
                    {error, Reason}
            end;
        {error, disconnected} ->
            ?tp(warning, "omp_mysql_connector_query_failed", LogInfo#{reason => disconnected}),
