...
if [[ ! -f "$PLUGIN_TAR" ]]; then
    ...
    echo "[redis] Run: make plugin-$PLUGIN_APP" >&2
    exit 1
fi
...
echo "[redis] also publishing a non-matching topic that should NOT be replayed"
mqttx pub -h 127.0.0.1 -p "$EMQX_MQTT_PORT" -V 5 -q 1 \
    -t "other/topic" -m '{"backend":"redis","seq":2}' >/dev/null

echo "[redis] restarting EMQX (Redis keeps the persisted message)"
docker compose stop emqx
docker compose start emqx
wait_api

echo "[redis] starting plugin on the fresh node (emqx_resource must be up first)"
plugin_start_attempts=20
while ((plugin_start_attempts > 0)); do
    http_code="$(curl -sS -o /dev/null -w '%{http_code}' \
        -X PUT "$BASE_URL/api/v5/plugins/$PLUGIN/start" \
        -u "$API_KEY:$API_SECRET" || true)"
    if [[ "$http_code" == "204" || "$http_code" == "200" ]]; then
        break
    fi
    sleep 1
    ((plugin_start_attempts--))
done
if ((plugin_start_attempts == 0)); then
    echo "[redis] FAIL: could not start plugin after restart" >&2
    exit 1
fi
sleep 2

echo "[redis] subscribing in a new session; expecting replay of offline/redis"
SUB_FILE="$(mktemp)"
...
echo "[redis] verifying non-matching topic is not replayed after restart"
SUB_FILE="$(mktemp)"
