...
set_redis_ssl_enabled(Config0) ->
    ...
    %% The set the docker-compose TLS services use, generated by
    %% `scripts/ct/gen-compose-certs.sh' before the services start.
    CertsDir = filename:join([
        emqx_common_test_helpers:proj_root(), ".ci", "docker-compose-file", "certs"
    ]),
    Config2 = emqx_utils_maps:deep_put(
        [<<"redis">>, <<"ssl">>, <<"cacertfile">>], Config1, filename:join(CertsDir, "cacert.pem")
    ),
    Config3 = emqx_utils_maps:deep_put(
        [<<"redis">>, <<"ssl">>, <<"certfile">>],
        Config2,
        filename:join(CertsDir, "client-cert.pem")
    ),
    emqx_utils_maps:deep_put(
        [<<"redis">>, <<"ssl">>, <<"keyfile">>], Config3, filename:join(CertsDir, "client-key.pem")
    ).
