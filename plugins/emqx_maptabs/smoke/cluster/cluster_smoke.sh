...
## catch-up on a 2-node emqx/emqx-enterprise:5.10.4 cluster.
##
## Prerequisites:
...
##
## What it does:
##   1) Builds the plugin tarball ('make compile-emqx-enterprise' +
##      'make plugin-emqx_maptabs').
...
build_plugin() {
    ...
    (cd "$ROOT_DIR" && make compile-emqx-enterprise && make "plugin-$PLUGIN_APP") \
...
## cluster_rpc replication to the peer is asynchronous only across a
## node restart; in steady state it is synchronous, but retrying keeps
## the assertion race-free.
