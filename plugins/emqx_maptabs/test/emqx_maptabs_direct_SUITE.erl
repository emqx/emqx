...
-doc "Lookup arities, exact-term key matching, and the rule SQL entrypoints.".
...
-doc "Fail-closed JSON validation and table/file name rules.".
...
-doc "Size/row/table limits guard the load entrypoint; invalid config falls back to defaults.".
t_limits(_Config) ->
...
-doc "Error returns for unreadable, non-JSON, and unknown-table inputs.".
...
-doc "A stopped plugin degrades to misses; storage stays writable and is loaded at restart.".
...
-doc "Unexpected gen_server messages and remote-call error mapping.".
...
-doc "Plugin application callbacks and rule-function registration.".
...
-doc "Every CLI command on the happy path and on error inputs.".
t_cli(_Config) ->
    mock_ctl_print(),
...
-doc "maptab_lookup resolves through the rule SQL tester.".
