...
-module(emqx_agent_tool_helpers).
...
%% The caller merges the returned map with tool-specific fields.
...
format_error({missing_tools, Refs}) when is_list(Refs) ->
    iolist_to_binary(["missing referenced tools: ", lists:join(", ", Refs)]);
format_error(Reason) ->
...
%% Build and publish a unary tool reply to `$cap/<Type>/<ToolId>/response/<ReqId>`.
-spec publish_reply(binary(), binary(), map(), map()) -> ok.
publish_reply(Type, ToolId, Request, Data) ->
