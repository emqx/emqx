...
on_message_publish(#message{topic = <<"cap/", Rest/binary>>, payload = Payload} = Msg) ->
    case binary:split(Rest, <<"/">>, [global]) of
        [Type, SkillId, <<"request">>] ->
            dispatch(Type, SkillId, Payload);
        _ ->
            ok
    end,
    {ok, Msg};
...
dispatch(Type, SkillId, Payload) ->
    case emqx_agent_skill_registry:lookup(Type, SkillId) of
        {ok, #{module := Module, context := Context}} ->
            Request = emqx_utils_json:decode(Payload),
            Module:handle_invoke(SkillId, Context, Request);
        {ok, #{module := Module}} ->
            Request = emqx_utils_json:decode(Payload),
            Module:handle_invoke(SkillId, #{}, Request);
        {error, not_found} ->
            ok
    end.
