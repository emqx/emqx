...
-module(emqx_agent_tool_postgresql_connector).
...
resource_type() -> agent_tool_pgsql.
...
on_start(
    ...
    ?SLOG(info, #{
        msg => "starting_agent_tool_postgresql_connector",
        connector => InstId,
...
on_stop(InstId, State) ->
    ?SLOG(info, #{
        msg => "stopping_agent_tool_postgresql_connector",
        connector => InstId
    }),
...
parse2(Conn, Name, SQL, Types) ->
    epgsql:sync_on_error(
        ...
        epgsql_sock:sync_command(
            Conn, emqx_agent_tool_postgresql_cmd_parse2, {Name, SQL, Types}
        )
    ).
