...
-module(emqx_agent_tool_postgresql).
...
Tool instances differ by SQL query template, schemas, and selected connection.
""".
...
resource_id(ConnectionId) ->
    emqx_agent_tool_connections:resource_id(ConnectionId).

-spec init() -> ok.
init() ->
    ok = emqx_agent_tool_registry:register_type(?TOOL_TYPE, ?MODULE),
    ok.

-spec deinit() -> ok.
deinit() ->
    ok = emqx_agent_tool_registry:unregister_type(?TOOL_TYPE),
    ok.
...
create(
    ...
    case validate_connection(ConnectionId) of
        ...
        {error, _} = Error ->
            ?SLOG(error, #{
                msg => "postgresql_tool_create_failed",
                tool_id => ToolId,
                connection_id => ConnectionId,
                reason => Error
            }),
...
-spec to_map(map()) -> map().
to_map(#{
    tool_id := Id, description := Desc, context := Ctx, input_schema := In
}) ->
    #{
        <<"tool_id">> => Id,
        <<"type">> => ?TOOL_TYPE,
    ...
    case emqx_agent_config:lookup_connection(ConnectionId) of
        {ok, Connection} ->
            case is_postgresql(Connection) of
                true ->
                    ?SLOG(info, #{
                        msg => "postgresql_tool_connection_validated",
                        connection_id => ConnectionId
                    }),
                    ok;
                false ->
                    ?SLOG(error, #{
                        msg => "postgresql_tool_connection_invalid_type",
                        connection_id => ConnectionId,
        ...
        {error, not_found} ->
            ?SLOG(error, #{
                msg => "postgresql_tool_connection_not_found",
                connection_id => ConnectionId
            }),
