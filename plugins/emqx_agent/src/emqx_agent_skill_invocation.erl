...
-module(emqx_agent_skill_invocation).
...
start_link(Type, SkillId, Module, Context, Request, Timeout) ->
    gen_server:start_link(?MODULE, {Type, SkillId, Module, Context, Request, Timeout}, []).

init({Type, SkillId, Module, Context, Request, Timeout}) ->
...
handle_info(invoke_timeout, #state{exec_pid = Pid, timer_ref = TRef} = State) ->
    ...
    ?SLOG(warning, #{
        msg => "skill_invocation_timeout",
        type => State#state.type,
        skill_id => State#state.skill_id
    }),
...
publish_reply(State, ReplyData) ->
    emqx_agent_skill_helpers:publish_reply(
        State#state.type,
        State#state.skill_id,
        State#state.request,
        ReplyData
    ).

format_reason(Reason) ->
    emqx_agent_skill_helpers:format_error(Reason).
