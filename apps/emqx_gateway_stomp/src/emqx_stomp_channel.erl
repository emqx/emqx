...
%% Bounds for the state a connection may accumulate for transactions.
%%
%% These mirror the `gateway.stomp.transaction` schema defaults on purpose: they
%% are the fallback when the config path is absent, and a live channel must not
%% crash because of a missing default.
...
frame_bytes(#stomp_frame{command = Command, headers = Headers, body = Body}) ->
    iolist_size(Command) +
        iolist_size(Body) +
