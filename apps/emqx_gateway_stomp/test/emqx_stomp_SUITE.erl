...
reset_stomp_transaction() ->
    Defaults = default_transaction_conf(),
    RawConfig = emqx_config:get_raw([gateway, stomp]),
    case maps:get(<<"transaction">>, RawConfig, Defaults) of
        Defaults -> ok;
        _ -> update_stomp_transaction(Defaults)
    end.
