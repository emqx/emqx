...
-module(emqx_ocpp_connection_SUITE).
...
t_get_peer_empty_header_names(_Config) ->
    ?assertEqual(?SOCK_PEER, emqx_ocpp_connection:get_peer(req(), opts("", ""))).
...
t_get_peer_configured_header_names(_Config) ->
    ?assertEqual(
        ...
        emqx_ocpp_connection:get_peer(req(), opts("X-Forwarded-For", "X-Forwarded-Port"))
    ).
...
t_parse_sec_websocket_protocol_unparseable(_Config) ->
    ...
    ?assertEqual(
        ...
        emqx_ocpp_connection:parse_sec_websocket_protocol([req(), Opts, ws_opts()], st)
    ),
    ...
    ?assertEqual(
        ...
        emqx_ocpp_connection:parse_sec_websocket_protocol([req(), Opts1, ws_opts()], st)
    ),
