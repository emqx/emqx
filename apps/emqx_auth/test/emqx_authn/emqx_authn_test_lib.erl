...
client_ssl_cert_opts() ->
    ...
    #{
        <<"keyfile">> => iolist_to_binary(filename:join(Dir, "client-key.pem")),
        <<"certfile">> => iolist_to_binary(filename:join(Dir, "client-cert.pem")),
        <<"cacertfile">> => iolist_to_binary(filename:join(Dir, "cacert.pem"))
    }.
