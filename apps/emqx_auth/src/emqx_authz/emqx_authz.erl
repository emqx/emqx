...
do_authorize(
    AuthzContext, Action, Topic, [#{precondition := Precondition} = SourceState | SourceStates]
) ->
    case check_precondition(Precondition, AuthzContext, Action, Topic) of
        true ->
...
do_authorize(AuthzContext, Action, Topic, [SourceState | SourceStates]) ->
    do_authorize_with_source(AuthzContext, Action, Topic, SourceState, SourceStates).
