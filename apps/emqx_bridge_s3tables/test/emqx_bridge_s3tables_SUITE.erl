...
t_aggreg_upload_restart_corrupted(Config0) ->
    ...
    MessageCheckFn = fun(Context) ->
        ...
        ?assertEqual(
            lists:sort(Expected),
            %% Some rows might've been written already.
            lists:usort(RowsStr),
            #{expected => Expected}
        ),
        ok
...
t_upload_failure_during_process_write(Config) ->
    do_t_upload_failure(_S3UploadFnName = write, Config).
...
t_upload_failure_during_process_complete(Config) ->
    do_t_upload_failure(_S3UploadFnName = complete, Config).

do_t_upload_failure(S3UploadFnName, Config) ->
    ...
    case IsPartitioned of
        not_partitioned ->
            ?assertMatch(
                #{
                    error := {unhealthy_target, {data_file, <<"mocked error">>}},
        ...
        partitioned ->
            ?assertMatch(
                #{
                    error := {unhealthy_target, {data_file, _PK, <<"mocked error">>}},
                    status := ?status_disconnected
                },
                emqx_bridge_v2_testlib:health_check_channel(Config)
            )
