...
-module(emqx_bridge_redis_single_schema).

-include_lib("typerefl/include/types.hrl").
-include_lib("hocon/include/hoconsc.hrl").
-define(TYPE, redis_single).
-define(TYPE_NAME, redis_single_producer).
...
fields(action) ->
    {?TYPE_NAME,
        ?HOCON(
            ?MAP(name, ?R_REF(emqx_bridge_redis, "action_redis_single")),
            #{
                desc => <<"Redis Single Producer Action Config">>,
                required => false
            }
        )};
%%=========================================
...
fields("put_bridge_v2") ->
    emqx_bridge_redis:fields("action_redis_single");
fields("get_single") ->
    emqx_bridge_schema:status_fields() ++ fields("put_single");
fields("put_single") ->
    fields("config_connector");
fields("post_single") ->
    emqx_bridge_redis:type_name_fields(?TYPE_NAME) ++ fields("put_single").
...
connector_example(put) ->
    #{
        enable => true,
        desc => <<"My Redis Single Connector">>,
