...
-module(emqx_connector_aggregator_utils_tests).

-include_lib("eunit/include/eunit.hrl").

accept_legacy_datetime_test() ->
    Template = <<"${action}/${node}/${datetime.rfc3339utc}/${sequence}">>,
    ?assertEqual(ok, emqx_connector_aggregator_utils:validate_key_template(Template)).
...
reject_unknown_datetime_token_test() ->
    ...
    ?assertMatch({error, _}, emqx_connector_aggregator_utils:validate_key_template(Template)).
