...
t_disable_reenable(Config) ->
    ...
    [TN1, TN2] = TargetNodes = ?config(target_nodes, Config),
    SourceName = <<"cl.target">>,
    SourceC1 = emqx_cluster_link_SUITE:start_client(<<"sc1">>, SN1),
    TargetC1 = emqx_cluster_link_SUITE:start_client(<<"tc1">>, TN1),
    TargetC2 = emqx_cluster_link_SUITE:start_client(<<"tc2">>, TN2),
    ...
    %% kicks in.
    ok = disable_and_force_gc(source, SourceName, SourceLink1, Config, #{
        expected_num_route_deletions => 2
    }),
    {ok, SRef1} = snabbkaffe:subscribe(
        ?match_event(#{?snk_kind := "clink_router_syncer_reset_requested"}),
        length(TargetNodes),
        infinity
    ),
    {ok, SRef2} = snabbkaffe:subscribe(
        ?match_event(#{?snk_kind := "clink_router_bootstrapped"}),
        length(TargetNodes),
        infinity
    ),
    ?assertMatch(
    ...
    ct:pal("waiting for routers to reset..."),
    {ok, _} = snabbkaffe:receive_events(SRef1),
    ct:pal("waiting for routers to bootstrap..."),
    {ok, _} = snabbkaffe:receive_events(SRef2),
