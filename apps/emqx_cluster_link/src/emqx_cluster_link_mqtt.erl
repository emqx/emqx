...
publish_actor_init_sync(ClientPid, ReqId, RespTopic, TargetCluster, Actor, Incarnation) ->
    ...
    emqtt:publish(ClientPid, ?ROUTE_TOPIC, Properties, encode_payload(Payload), [{qos, ?QOS_1}]).
...
publish_route_sync(ClientPid, Actor, Incarnation, Updates) ->
    ...
    emqtt:publish(ClientPid, ?ROUTE_TOPIC, encode_payload(Payload), ?QOS_1).
...
publish_heartbeat(ClientPid, Actor, Incarnation) ->
    ...
    emqtt:publish_async(
        ClientPid, ?ROUTE_TOPIC, encode_payload(Payload), ?QOS_0, {fun(_) -> ok end, []}
    ).
