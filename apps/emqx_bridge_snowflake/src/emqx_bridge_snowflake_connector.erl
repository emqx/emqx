...
create_action(
    ...
    maybe
        {ok, SetupPoolState, WritePoolState, ChanPoolState} ?=
            start_streaming_pools(ActionResId, ActionConfig, ConnState),
...
start_streaming_pools(ActionResId, ActionConfig, ConnState) ->
    maybe
        {ok, Hostname, SetupPoolState} ?=
            init_streaming_setup_pool(ActionResId, ActionConfig, ConnState),
        {ok, WritePoolState} ?=
            init_streaming_write_pool(ActionResId, ActionConfig, Hostname),
        {ok, ChanPoolState} ?=
            init_streaming_channel_pool(ActionResId, ActionConfig, Hostname, SetupPoolState),
        {ok, SetupPoolState, WritePoolState, ChanPoolState}
    end.
...
open_channels(ActionResId) ->
    ...
    emqx_utils:foldl_while(
        fun({_, Worker}, _Acc) ->
            ...
            case emqx_bridge_snowflake_channel_client:open_channel(ChanClient) of
                ok ->
                    {cont, ok};
                {error,
...
streaming_destroy_allocated_resources(ConnResId, ActionResId) ->
    maps:foreach(
        fun
            (?streaming_channel_pool(Id), _) when
                ActionResId == '_' orelse Id == ActionResId
            ->
                ChanPoolId = streaming_channel_pool(ActionResId),
                _ = emqx_resource_pool:stop(ChanPoolId),
                ok;
            (?streaming_write_pool(Id), _) when
                ActionResId == '_' orelse Id == ActionResId
            ->
                WritePoolId = streaming_write_pool(ActionResId),
                _ = ehttpc_sup:stop_pool(WritePoolId),
                ok;
            (?streaming_setup_pool(Id), _) when
                ActionResId == '_' orelse Id == ActionResId
            ->
                SetupPoolId = streaming_setup_pool(ActionResId),
                _ = ehttpc_sup:stop_pool(SetupPoolId),
                ok;
            (?streaming_jwt_tab(Id), ChanJWTTab) when
                ActionResId == '_' orelse Id == ActionResId
            ->
