...
-define(streaming_setup_pool, streaming_setup_pool).
...
on_start(ConnResId, ConnConfig) ->
    case init_streaming_setup_pool(ConnResId, ConnConfig) of
        {ok, State0} ->
...
on_get_status(ConnResId, ConnState) ->
    ...
    SetupPoolId = streaming_setup_pool(ConnResId),
    maybe
...
start_streaming_pools(ConnResId, ActionResId, ActionConfig, ConnState) ->
    maybe
        {ok, WritePoolState} ?=
            init_streaming_write_pool(ConnResId, ActionResId, ActionConfig, ConnState),
        {ok, ChanPoolState} ?=
            init_streaming_channel_pool(ConnResId, ConnState, ActionResId, ActionConfig),
        {ok, WritePoolState, ChanPoolState}
    end.

init_streaming_setup_pool(ConnResId, ConnConfig) ->
    #{
        account := Account,
        connect_timeout := ConnectTimeout,
    ...
    allocate(ConnResId, ?streaming_setup_pool, SetupPoolId),
    maybe
...
streaming_setup_pool(ResId) -> <<ResId/binary, ":setup">>.
streaming_write_pool(ResId) -> <<ResId/binary, ":write">>.
streaming_channel_pool(ResId) -> <<ResId/binary, ":channel">>.
    ...
    maps:foreach(
        fun
            ...
            ->
                ...
                deallocate(ConnResId, Key),
                ok;
            (?streaming_setup_pool = Key, SetupPoolId) when
                ActionResId == '_'
            ->
                _ = ehttpc_sup:stop_pool(SetupPoolId),
                deallocate(ConnResId, Key),
