...
-define(WWW_AUTHENTICATE_FULL,
    <<"Basic realm=\"emqx-dashboard\", Bearer realm=\"emqx-dashboard\"">>
).
...
-define(WWW_AUTHENTICATE_BEARER, <<"Bearer realm=\"emqx-dashboard\"">>).
...
unauthorized_markdown() ->
    Lines = [
        <<"# ", (api_title())/binary, " - Authentication Required\n\n">>,
        <<
...
api_title() ->
    iolist_to_binary([emqx_release:description(), " API"]).

release_version() ->
    iolist_to_binary(emqx_release:version()).

server_url() ->
    iolist_to_binary(emqx_dashboard_swagger:base_path()).
