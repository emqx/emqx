...
create(#{} = Config) ->
    %% Validate before destructuring: the fields below all carry schema defaults,
    %% so a config that went through the schema has them, but hand-built configs
    %% (e.g. tests that only exercise the ssl check) may not. Running the check
    %% first keeps it reachable for those.
