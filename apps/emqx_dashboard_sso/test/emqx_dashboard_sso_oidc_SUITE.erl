...
%% Masking must only replace the `file' leaf, not the whole `client_jwks'
%% object, or the value stops matching the `client_file_jwks' union member and
%% PUT rejects it (see `t_client_jwks_update_roundtrip').
t_client_jwks_redaction(TCConfig) ->
    ...
    ParamsWithJwks = ProviderParams#{
        <<"client_jwks">> => #{<<"type">> => <<"file">>, <<"file">> => jwks_content()}
    },
    ?assertMatch(
...
t_client_jwks_update_roundtrip(TCConfig) ->
    start_apps(?FUNCTION_NAME, TCConfig),
    Node = node(),
    JwksContent = jwks_content(),
    ParamsWithJwks = (oidc_provider_params())#{
...
%% placeholder means "unchanged"; an explicit `none' means "remove", and the
