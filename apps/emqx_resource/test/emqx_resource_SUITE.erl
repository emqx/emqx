...
t_fallback_actions(Config) when is_list(Config) ->
    ...
    IsSimple =
        case re:run(atom_to_binary(ChanQueryMode), <<"simple">>, [{capture, none}]) of
            match -> simple;
            nomatch -> buffered
        end,
    #{
    ...
    MkEventMatcher = fun(RequestQueryKind) ->
        ...
        case {ShouldBatch, CallbackMode, QMSummary} of
            ...
            {no_batch, async_if_possible, #{has_internal_buffer := true}} ->
                ?match_event(#{?snk_kind := call_query_async});
            {no_batch, always_sync, #{has_internal_buffer := true}} ->
                ?match_event(#{?snk_kind := call_query_sync});
        ...
        begin
            ...
            emqx_utils:pforeach(
                fun(N) ->
                    ...
                    Ctx = #{req_n => N},
                    RequestQueryKind = sync,
                    EventMatcher = MkEventMatcher(RequestQueryKind),
                    {Result, {ok, _}} =
                        snabbkaffe:wait_async_action(
                            fun() ->
                                fallback_action_query(ConnResId, ChanId, Oracle, Ctx, QueryOpts#{
                    ...
                    ct:pal("~p received reply ~b", [self(), N]),
                    ?assertEqual(
                        {error, {unrecoverable_error, fallback_time}},
                        Result
                    )
...
fallback_action_query(ConnResId, ChanId, Oracle, Ctx, #{query_mode := async} = QueryOpts) ->
    Alias = alias([reply]),
    ReplyTo = {fun(ReplyCallerContext) -> Alias ! {Alias, ReplyCallerContext} end, []},
    ct:pal("~p sending query\n  ~p", [self(), #{query_opts => QueryOpts, ctx => Ctx}]),
    Res = emqx_resource:query(
        ChanId,
        {ChanId, #{q => ask, fn => Oracle, ctx => Ctx}},
        maps:merge(
            #{connector_resource_id => ConnResId},
            QueryOpts#{reply_to => ReplyTo}
        )
    ),
    case Res of
        ok ->
    ...
    receive
        {Alias, ReplyCallerContext} ->
            ct:pal("~p received async reply caller context:\n  ~p", [self(), ReplyCallerContext]),
            #{result := Result} = ReplyCallerContext,
            Result
    after 10_000 -> ct:fail("~p timed out waiting for reply", [self()])
    end;
fallback_action_query(ConnResId, ChanId, Oracle, Ctx, #{query_mode := sync} = QueryOpts) ->
    ct:pal("~p sending query\n  ~p", [self(), #{query_opts => QueryOpts, ctx => Ctx}]),
    emqx_resource:query(
        ChanId,
        {ChanId, #{q => ask, fn => Oracle, ctx => Ctx}},
