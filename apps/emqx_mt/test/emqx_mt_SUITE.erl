...
`null`, `none`) is denied even under `allow_only_managed_namespaces = false`,
while a client with a non-reserved pre-auth tns still passes through.
""".
...
t_post_auth_reserved_tns_empty_render(_Config) ->
    ...
    %% `mqtt.client_attrs_init' maps `username' to `tns', so a reserved
    %% username makes the pre-auth `tns' reserved; the expression renders
    %% empty, and the leftover reserved tns must be denied.
    lists:foreach(
        fun(Ns) ->
            ClientId = ?NEW_CLIENTID(),
            ?assertError(
    ...
    %% Regression guard: a non-reserved pre-auth tns is stripped and the
...
`client_attrs.tns` is a reserved namespace name is denied even under
`allow_only_managed_namespaces = false`, while a client with a non-reserved
