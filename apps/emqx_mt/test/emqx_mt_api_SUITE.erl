...
Older EMQX versions don't have the newer `subscribes` limiter, so they export mnesia
records for `emqx_mt_config` lacking those sub-keys.
...
t_import_backup_lacking_limiter_config(_TCConfig) ->
    ...
    ?assertMatch(
        {200, #{~"limiter" := #{~"client" := #{~"subscribes" := _}}}},
    ...
    #{limiter := #{client := #{subscribes := _}}} = Cfgs0,
    Cfgs = emqx_utils_maps:deep_remove([limiter, client, subscribes], Cfgs0),
    ...
    ?assertMatch(
        ...
        }} when not is_map_key(~"subscribes", ClientLimiter),
        get_managed_ns_config(Ns)
    ),
