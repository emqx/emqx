...
simple_schema2() ->
    #{
        ...
        <<"fields">> => [
            ...
            %% FIXME: check what kind of inputs pyiceberg/pyarrow accept for this
            %% #{
            %%     <<"id">> => 4,
            %%     <<"name">> => <<"col_decimal">>,
            %%     <<"type">> => <<"decimal(10,2)">>,
...
simple_setup_table(TestCase) ->
    ...
    Namespace = [TestCaseBin],
    Table = TestCaseBin,
    Schema = simple_schema2(),
...
t_rule_action(Config) when is_list(Config) ->
    ...
    PublishFn = fun(Context) ->
        Payloads = lists:map(fun(_) -> PayloadFn() end, lists:seq(1, NumPayloads)),
        emqx_utils:pforeach(
            fun(P0) ->
    ...
    PostPublishFn = fun(Context) ->
        ...
        [P1, P2, P3] = lists:map(fun(#{<<"str">> := S}) -> S end, Payloads),
        ?block_until(#{?snk_kind := connector_aggreg_delivery_completed}),
    ...
    Opts = #{
        ...
        sql => render(
            <<
                ...
                "  payload.decimal as col_decimal, "
