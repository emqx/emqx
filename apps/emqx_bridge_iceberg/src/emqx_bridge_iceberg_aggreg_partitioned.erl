...
to_partition_keys(Record, PartitionFields) ->
    lists:map(
        fun(#{get_fn := GetFn, transform_fn := TransformFn}) ->
            TransformFn(GetFn(Record))
        end,
        PartitionFields
    ).
