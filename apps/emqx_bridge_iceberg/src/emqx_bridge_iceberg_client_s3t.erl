...
-spec load_table(t(), string(), string()) -> {ok, map()} | {error, term()}.
...
update_table(Client, Params) ->
        ...
        <<"updates">> => [
            #{
                ...
                <<"snapshot">> => #{
                    ...
                    %% TODO: FIXME: handle parent snapshot id.....
                    %% <<"parent-snapshot-id">> => null,
                    <<"sequence-number">> => SeqNum,
                    <<"timestamp-ms">> => erlang:system_time(millisecond),
    ...
    Namespace = join_ns(NamespaceList),
    Context = #{
        ?method => Method,
        ?path_parts => [arn, "namespaces", Namespace, "tables", Table],
        ?query_params => [],
...
sign_request(Client, Context) ->
    %% TODO: ensure base uri does not end in trailing slash
    #{
        ?host := Host,
        ?aws_config := AWSConfig
    } = Client,
