...
init_per_suite(Config) ->
    ...
    emqx_common_test_helpers:start_apps([emqx_st_statistics]),
    Config.

end_per_suite(Config) ->
    emqx_common_test_helpers:stop_apps([emqx_st_statistics]),
    Config.
...
t_log_and_pub(_) ->
    ...
    Now = ?NOW,
    %% publish
    ?assertEqual(ets:info(?LOG_TAB, size), 0),
    lists:foreach(fun(I) ->
    ...
    case ets:info(?LOG_TAB, size) of
        5 ->
            ok;
        _ ->
            ?assertNotEqual(ets:info(?TOPK_TAB, size), 0)
    end,

    timer:sleep(2400),

    ?assertEqual(ets:info(?LOG_TAB, size), 0),
    ?assertEqual(ets:info(?TOPK_TAB, size), 3),
    try_receive(3),
    try_receive(2),
