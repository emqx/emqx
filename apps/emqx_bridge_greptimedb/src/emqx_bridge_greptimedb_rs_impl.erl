...
on_start(ConnResId, ConnConfig) ->
    ...
    ClientOpts = #{
        ...
        pool_type => random,
        %% FIXME: lib needs to consider this....
        auto_reconnect => ?AUTO_RECONNECT_S,
        endpoints => [bin(Server)],
        dbname => Database,
        username => Username,
        %% TODO: teach lib to unwrap secrets itself...
...
do_insert_async(ChanResId, TablesToPoints, ConnState, ReplyFnAndArgs0) ->
    ...
    %% fixme: lib should return pid of worker.....
    ok = greptimedb_rs:insert_async(Client, Table, Points, ReplyFnAndArgs),
    {ok, self()}.
...
handle_async_result_continuation(Context0, Result, NextTable, PointsWithIndices) ->
    ...
    ReplyFnAndArgs = {fun ?MODULE:reply_callback/2, [Context]},
    %% fixme: lib should return pid of worker.....
    ok = greptimedb_rs:insert_async(Client, NextTable, Points, ReplyFnAndArgs),
    ok.
