...
%%
%% Invoke topic:  cap/invoke/agent.create_skill/<skill_id>/request
%% Reply  topic:  cap/invoke/agent.create_skill/<skill_id>/response/<req_id>
...
on_message_publish(
    #message{topic = <<"cap/invoke/agent.create_skill/", Rest/binary>>, payload = Payload} = Msg
) ->
