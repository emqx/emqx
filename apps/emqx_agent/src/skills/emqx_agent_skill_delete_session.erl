...
%% Management skill: delete a session profile.
%%
%% Refuses deletion if the profile is referenced in any pipeline
%% llm_loop step's session_profile field.
%%
%% Args:
%%   name — profile name  (required)
%%
%% Invoke topic:  cap/invoke/agent.delete_session/<skill_id>
...
-define(SKILL_TYPE, <<"agent.delete_session">>).
...
-define(OUTPUT_SCHEMA, #{
    ...
    <<"properties">> => #{
        ...
        <<"used_by">> => #{
            <<"type">> => <<"array">>,
            <<"description">> => <<"Pipeline ids that reference this profile">>
        }
    },
    <<"required">> => [<<"status">>]
}).
...
create(#{skill_id := SkillId}) ->
    emqx_agent_skill_registry:register(#{
        skill_id => SkillId,
        type => ?SKILL_TYPE,
        display_name => <<"Delete Session Profile">>,
        description =>
            <<"Delete a session profile. Refused if the profile is used in any pipeline.">>,
...
on_message_publish(
    #message{topic = <<"cap/invoke/agent.delete_session/", SkillId/binary>>, payload = Payload} =
        Msg
) ->
...
do_delete(#{<<"name">> := Name}) ->
    case emqx_agent_service:profile_delete(Name) of
        ok ->
            #{<<"status">> => <<"ok">>};
        {error, not_found} ->
            #{<<"status">> => <<"error">>, <<"reason">> => <<"session profile not found">>};
        {error, {in_use, PipelineIds}} ->
            Joined = join_ids(PipelineIds),
            #{
                <<"status">> => <<"error">>,
                <<"reason">> => <<"session profile is used in pipeline(s): ", Joined/binary>>,
                <<"used_by">> => PipelineIds
            }
    end;
do_delete(_) ->
    #{<<"status">> => <<"error">>, <<"reason">> => <<"missing required field: name">>}.
