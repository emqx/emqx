...
def main() -> int:
    ...
    removed = api_delete(f"/agent/pipelines/{PIPELINE_ID}")
    print(f"  pipeline {PIPELINE_ID!r}: {'removed' if removed else 'not found'}")

    print("==> Removing session profile")
    removed = api_delete(f"/agent/session_profiles/{PROFILE_NAME}")
    ...
    for skill_type, skill_id in [
        ...
        removed = api_delete(f"/agent/skills/{skill_type}/{skill_id}")
