...
t_client_attrs_with_control_chars_dropped(_) ->
    ...
    {ok, Good} = emqx_variform:compile(<<"concat(['tenant-a'])">>),
    {ok, Bad} = emqx_variform:compile(<<"concat(['tenant-a\r\nsmuggled'])">>),
    Result = emqx_channel:initialize_client_attrs(
        [
    ...
    ?assertEqual(#{<<"good">> => <<"tenant-a">>}, maps:get(client_attrs, Result, #{})).
