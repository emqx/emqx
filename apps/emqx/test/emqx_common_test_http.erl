...
-define(DEFAULT_API_NAME, <<"default_api_name">>).
-define(DEFAULT_API_SECRET, <<"default_api_secret">>).
...
default_auth_header() ->
    {ok, #{api_key := APIKey}} = emqx_mgmt_auth:read(?DEFAULT_API_NAME),
    auth_header(
        erlang:binary_to_list(APIKey), erlang:binary_to_list(?DEFAULT_API_SECRET)
    ).
...
create_default_app() ->
    ...
    emqx_mgmt_auth:create(
        ?DEFAULT_API_NAME, ?DEFAULT_API_SECRET, true, ExpiredAt, <<"default api key for test">>
    ),
    ok.

delete_default_app() ->
    emqx_mgmt_auth:delete(?DEFAULT_API_NAME).
