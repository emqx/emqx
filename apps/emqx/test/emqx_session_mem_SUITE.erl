...
t_deliver_drops_expired_qos2(_) ->
    Session = session(),
    [Msg0] = enrich(delivery(?QOS_2, <<"t/expired">>, <<"payload">>, _Expiry = 1), Session),
    Msg = Msg0#message{timestamp = emqx_message:timestamp(Msg0) - timer:seconds(2)},
    {ok, [], Session1} = emqx_session_mem:deliver(clientinfo(), [Msg], Session),
    ?assertEqual(0, emqx_session_mem:info(inflight_cnt, Session1)).
