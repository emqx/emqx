...
t_deregister_providers({init, Config}) ->
    AuthNType = {'password-based', 'built-in-database'},
    [{global, 'mqtt:global'},
     {"auth", AuthNType} | Config];

t_deregister_providers(Config) when is_list(Config) ->
    emqx_config_handler:add_handler([authentication], emqx_authentication),

    ok = ?AUTHN:register_providers([{?config("auth"), ?MODULE}]),
    ...
    ?assertEqual({ok, []}, ?AUTHN:list_chains()),
    ?assertMatch({ok, _}, ?AUTHN:create_chain(Global)),

    ?assertMatch({ok, _}, ?AUTHN:create_authenticator(Global, AuthenticatorConfig)),

    ?assertMatch(
       {ok, [#{id := ID}]},
       ?AUTHN:list_authenticators(Global)),

    ok = ?AUTHN:deregister_providers([?config("auth")]),

    ?assertMatch(
       {ok, []},
       ?AUTHN:list_authenticators(Global));

t_deregister_providers({'end', Config}) ->
    ?AUTHN:delete_chain(?config(global)),
    ok.
