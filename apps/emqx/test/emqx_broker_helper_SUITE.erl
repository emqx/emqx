...
t_shard_assignment_concurrent(_) ->
    ...
    %% ...which amounts to at most this ratio of live assignments in the end:
    ...
    %% Assigments should have no more than a "small tail" of shard assigments, when
    %% compared against a perfect assigment:
    MaxShardIdeal = floor(RLive * NWorkers * NOperations / SCapacity),
    ?assertMatch(
        NAssignments when NAssignments < SCapacity * 1.5,
        lists:sum([N || {Shard, N} <- maps:to_list(AccHist), Shard > MaxShardIdeal]),
        AccHist
    ).
