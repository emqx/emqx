...
t_first_packet_not_connect_plaintext(_Config) ->
    Port = emqx_common_test_helpers:select_free_port(tcp),
    Name = ?FUNCTION_NAME,
    ListenerId = emqx_listeners:listener_id(tcp, Name),
    ...
    with_listener(tcp, Name, Conf, fun() ->
        Reason = invalid_connect_packet,
        Count0 = proplists:get_value(
            Reason, emqx_listeners:shutdown_count(ListenerId, ListenOn), 0
        ),
        ...
        ?retry(
            ...
            10,
            begin
                Count1 = proplists:get_value(
                    Reason,
                    emqx_listeners:shutdown_count(ListenerId, ListenOn),
                    0
                ),
...
t_first_packet_not_connect_plaintext_socket_backend(_Config) ->
    Port = emqx_common_test_helpers:select_free_port(tcp),
    Name = ?FUNCTION_NAME,
    ListenerId = emqx_listeners:listener_id(tcp, Name),
    ListenOn = {{127, 0, 0, 1}, Port},
