...
t_quic_takeover_tls(Config) ->
    ...
    %% NOTE: emqx_cm:get_chan_info/1 reads from ETS which is updated asynchronously
    %% after emqtt:subscribe returns. Use retry to avoid a race condition where
    %% the cached session does not yet include the subscription.
