...
t_parse_connect_user_property_limit(_) ->
    ...
    ?assertMatch(
        {ok, _Packet, <<>>, _},
        emqx_frame:parse(make_v5_connect_frame(<<>>), Zero)
    ),
    ?ASSERT_FRAME_THROW(
    ...
    {none, LimitedOptions} = Limited,
    Limit = maps:get(max_connect_user_properties, LimitedOptions),
    ?assertMatch(
        {ok, _Packet, <<>>, _},
        emqx_frame:parse(make_v5_connect_frame(user_properties(Limit)), Limited)
    ),
    ?ASSERT_FRAME_THROW(
        #{cause := too_many_user_properties, limit := Limit},
        emqx_frame:parse(make_v5_connect_frame(user_properties(Limit + 1)), Limited)
    ),
    ...
    LargeConnectOpts = emqx_frame:initial_parse_state(#{
        max_connect_user_properties => Limit
    }),
    ?assertMatch({ok, _Packet, <<>>, _}, emqx_frame:parse(LargeConnect, LargeConnectOpts)).

%% Parser state created before the hot patch has no count-limit key. A missing
%% parser option means unlimited, and parsing does not mutate the old state map.
t_hot_patch_connect_user_property_limit(_) ->
    ...
    LegacyState = {none, LegacyOptions},
    {ok, _Packet, <<>>, {none, NOptions}} =
        emqx_frame:parse(
            make_v5_connect_with_will_frame(user_properties(20), user_properties(20)),
            LegacyState
        ),
    false = maps:is_key(max_connect_user_properties, NOptions).
