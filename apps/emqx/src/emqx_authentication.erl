...
handle_call({deregister_providers, AuthNTypes}, _From, #{providers := Providers} = State) ->
    AuthNProviders = maps:values(maps:with(AuthNTypes, Providers)),
    ok = delete_provider_authenticators(AuthNProviders),
...
delete_provider_authenticators(Providers) ->
    MatchFun = fun(#authenticator{provider = Provider}) ->
                       lists:member(Provider, Providers)
               end,
    Chains = ets:tab2list(?CHAINS_TAB),
    lists:foreach(fun(Chain) ->
                          do_delete_authenticators(MatchFun, Chain)
                  end,
                  Chains).
