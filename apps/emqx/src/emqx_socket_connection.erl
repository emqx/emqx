...
send(Num, IoVec, #state{socket = Socket, sockstate = idle} = State) ->
    ...
    case socket:sendv(Socket, IoVec, Handle) of
        ...
        {select, _Info} ->
            sent(Num, Oct, queue_send(Handle, IoVec, State));
        {error, {Reason, _Rest}} ->
...
send(Num, IoVec, #state{sockstate = SS = #congested{sendq = SQ, deadline = Deadline}} = State) ->
    case erlang:monotonic_time(millisecond) of
        BeforeDeadline when BeforeDeadline < Deadline ->
            NState = State#state{sockstate = SS#congested{sendq = [IoVec | SQ]}},
            sent(Num, iolist_size(IoVec), NState);
        _PastDeadline ->
...
handle_send_ready(Socket, SS = #congested{sendq = SQ}, State) ->
    ...
    case socket:sendv(Socket, IoVec, Handle) of
        ...
        {select, _Info} ->
            ...
            NState = State#state{sockstate = SS#congested{sendq = IoVec}},
            {ok, NState};
        {error, {Reason, _Rest}} ->
            %% Defer error handling:
            {ok, {sock_error, Reason}, State};
...
queue_send(Handle, IoVec, State = #state{sockstate = idle, listener = {Type, Name}}) ->
    Timeout = emqx_config:get_listener_conf(Type, Name, [tcp_options, send_timeout], 15_000),
    Deadline = erlang:monotonic_time(millisecond) + Timeout,
    SockState = #congested{handle = Handle, deadline = Deadline, sendq = [IoVec]},
...
sendq_to_iovec([IoVec | Rest], Acc) ->
    sendq_to_iovec(Rest, IoVec ++ Acc);
sendq_to_iovec([], Acc) ->
    Acc.
