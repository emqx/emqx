...
handle_info(
    ...
    {_NewSRSIds, S, SchedS} = emqx_persistent_session_ds_stream_scheduler:on_new_stream_event(
        Ref, S0, SchedS0
    ),
...
update_seqno(
    Track,
    PacketId,
    Session = #{id := SessionId, s := S0, inflight := Inflight0, stream_scheduler_s := SchedS0}
) ->
