...
do_start_listener(tcp, Name, Id, #{bind := ListenOn} = Opts) ->
    esockd:open_tcpsocket(
        Id,
        ListenOn,
        esockd_opts(Id, tcp, Name, emqx_socket_connection, Opts)
    );
...
do_update_listener(tcp, Name, OldConf, NewConf = #{bind := ListenOn}) ->
    Id = listener_id(tcp, Name),
    case maps:get(bind, OldConf) of
        ListenOn ->
            esockd:reset_options(
                {Id, ListenOn},
                esockd_opts(Id, tcp, Name, emqx_socket_connection, NewConf)
            );
        _Different ->
            %% TODO
    ...
    maps:to_list(
        case Type of
            ...
            ssl ->
                ...
                SSLOpts = inject_sni_fun(ListenerId, Opts0, SSLOpts0),
                Opts2#{
                    ssl_options => SSLOpts,
                    tcp_options => TcpOpts
                }
