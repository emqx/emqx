...
-spec list_client_subscriptions(node(), emqx_types:mtns(), emqx_types:clientid()) ->
    [{emqx_types:topic(), emqx_types:subopts()}]
    | emqx_rpc:badrpc().
list_client_subscriptions(Node, Mtns, ClientId) ->
    rpc:call(Node, emqx_broker, subscriptions, [Mtns, ClientId]).
