...
-spec lookup_client(
    node(), emqx_types:mtns(), {clientid, emqx_types:clientid()} | {username, emqx_types:username()}
) ->
    [emqx_cm:channel_info()] | {badrpc, _}.
lookup_client(Node, Mtns, Key) ->
    rpc:call(Node, emqx_cm, lookup_client, [Mtns, Key]).
...
-spec takeover_session(emqx_types:mtns(), emqx_types:clientid(), emqx_cm:chan_pid()) ->
    ...
    | {expired | persistent, emqx_session:session()}
    | {badrpc, _}.
takeover_session(Mtns, ClientId, ChanPid) ->
    rpc:call(node(ChanPid), emqx_cm, takeover_session, [Mtns, ClientId, ChanPid], ?T_TAKEOVER * 2).

-spec kick_session(kick | discard, emqx_types:mtns(), emqx_types:clientid(), emqx_cm:chan_pid()) ->
    ok | {badrpc, _}.
kick_session(Action, Mtns, ClientId, ChanPid) ->
    rpc:call(
        node(ChanPid), emqx_cm, do_kick_session, [Action, Mtns, ClientId, ChanPid], ?T_KICK * 2
    ).

-spec takeover_kick_session(emqx_types:mtns(), emqx_types:clientid(), emqx_cm:chan_pid()) ->
    ok | {badrpc, _}.
takeover_kick_session(Mtns, ClientId, ChanPid) ->
    rpc:call(
        node(ChanPid),
        emqx_cm,
        do_takeover_kick_session_v4,
        [Mtns, ClientId, ChanPid],
        ?T_TAKEOVER * 2
    ).
