...
-type state() :: #{
    listener := emqx_listeners:listener_id(),
    rate_limiter => emqx_limiter_client:t(),
    capacity_ref => atomics:atomics_ref()
}.

-type capacity() :: atomics:atomics_ref() | infinity.
...
-spec create_capacity(pos_integer() | infinity) -> capacity().
create_capacity(infinity) ->
    infinity;
...
create(Options = #{listener := ListenerID}) ->
    ...
    case Options of
        ...
        #{max_connections := _, capacity := ARef} -> S1#{capacity_ref => ARef};
        #{} -> S1
    end.
