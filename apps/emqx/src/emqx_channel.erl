...
%% A `client.subscribe` hook may return more or fewer topic filters than it was
%% given. The results are paired with the requested topic filters by position
%% while the count is unchanged, and by topic otherwise.
gen_reason_codes(TFChecked, TFSubedWitNhRC) ->
    case do_gen_reason_codes([], TFChecked, TFSubedWitNhRC) of
        {ok, ReasonCodes} ->
            ReasonCodes;
        count_changed ->
            gen_reason_codes_by_topic(TFChecked, TFSubedWitNhRC)
    end.
...
do_gen_reason_codes(_Acc, [{_, ?RC_SUCCESS} | _RestTF], []) ->
    count_changed;
do_gen_reason_codes(
    Acc,
    [{_, InitialRC} | Rest],
...
%% Each topic filter passed to the hook takes the reason code of the subscribed
%% one with the same topic, or `RC_UNSPECIFIED_ERROR` if there is none, as its
%% initial `RC_SUCCESS` would read as granted QoS 0. A topic filter the hook
%% added gets no reason code: SUBACK carries one per topic filter of the
%% SUBSCRIBE packet, ordered as received (MQTT-3.9.3-1).
gen_reason_codes_by_topic(TFChecked, TFSubedWithNRC) ->
    NRCByTopic = maps:from_list([{Topic, NRC} || {{Topic, _SubOpts}, NRC} <- TFSubedWithNRC]),
    lists:map(
        fun
            ({{Topic, _SubOpts}, ?RC_SUCCESS}) ->
                maps:get(Topic, NRCByTopic, ?RC_UNSPECIFIED_ERROR);
            ({_TopicFilter, InitialRC}) ->
                InitialRC
        end,
        TFChecked
    ).
