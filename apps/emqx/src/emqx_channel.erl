...
%% The two lists are not assumed to be of the same length: a `client.subscribe`
%% hook is free to return fewer or more topic filters than were checked.
do_gen_reason_codes(Acc, [], _TFSubedWitNhRC) ->
    %% A topic filter the hook added on top of the requested ones is subscribed,
    %% but gets no reason code: SUBACK carries one per topic filter of the
...
do_gen_reason_codes(
    ...
    %% Authorized, but the hook dropped it, so it was never subscribed. Keeping
    %% the initial `RC_SUCCESS` would report a granted QoS 0 subscription that
    %% does not exist, the two being both 16#00.
    do_gen_reason_codes([?RC_UNSPECIFIED_ERROR | Acc], RestTF, []);
do_gen_reason_codes(
    Acc,
    [{_, InitialRC} | Rest],
    RestWithNRC
