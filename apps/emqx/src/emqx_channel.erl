...
handle_in(SubPkt = ?SUBSCRIBE_PACKET(PacketId, Properties, _TopicFilters0), Channel) ->
    try
        run_fold(
            [
                ...
                fun(TFChecked, _) -> run_sub_hooks(TFChecked, Properties, Channel) end
            ],
            SubPkt,
            Channel
        )
...
process_pending_subscribe(TFChecked, Channel) ->
    lists:mapfoldl(
        fun
            ({{Topic, SubOpts}, pending}, ChanAcc) ->
                do_subscribe(Topic, SubOpts, ChanAcc);
            ({{_Topic, _SubOpts}, RC}, ChanAcc) ->
                {RC, ChanAcc}
        end,
        Channel,
        TFChecked
...
handle_info({subscribe, TopicFilters}, Channel) ->
    ...
    {_ReasonCodes, NChannel} = process_subscribe(NTopicFilters, Channel),
    {ok, NChannel};
...
check_subscribe(SubPkt, _Channel) ->
    case emqx_packet:check(SubPkt) of
        ok -> SubPkt;
        {error, RC} -> throw({disconnect, RC})
    end.

%%--------------------------------------------------------------------
%% Check Sub Authorization

check_sub_authzs(TopicFilters, #channel{clientinfo = ClientInfo}) ->
    DenyAction = emqx:get_config([authorization, deny_action], ignore),
    [do_check_sub_authz(ClientInfo, DenyAction, TF) || TF <- TopicFilters].

do_check_sub_authz(ClientInfo, DenyAction, TopicFilter = {Topic, _SubOpts}) ->
    ...
    RealTopic = emqx_topic:get_shared_real_topic(Topic),
    case emqx_access_control:authorize(ClientInfo, Action, RealTopic) of
        ...
        allow ->
            {TopicFilter, pending};
        deny when DenyAction == disconnect ->
            throw({disconnect, ?RC_NOT_AUTHORIZED});
        deny ->
            {TopicFilter, ?RC_NOT_AUTHORIZED}
    end.

%%--------------------------------------------------------------------
%% Check Sub Caps

check_sub_caps(TopicFilters, #channel{clientinfo = ClientInfo}) ->
    [do_check_sub_caps(ClientInfo, TFChecked) || TFChecked <- TopicFilters].

do_check_sub_caps(ClientInfo, TFChecked = {{Topic, SubOpts}, pending}) ->
...
do_check_sub_caps(_ClientInfo, TFChecked = {{_Topic, _SubOpts}, _RC}) ->
    TFChecked.

%%--------------------------------------------------------------------
%% Run Subscribe Hooks

run_sub_hooks(TFChecked, Properties, #channel{clientinfo = ClientInfo}) ->
    HookArgs = [ClientInfo, Properties],
    [do_run_sub_hooks(HookArgs, TF) || TF <- TFChecked].

do_run_sub_hooks(HookArgs, {TopicFilter = {_Topic, _SubOpts}, pending}) ->
    case run_hooks('client.subscribe', HookArgs, [TopicFilter]) of
        [NTopicFilter] ->
            {NTopicFilter, pending};
        [NTopicFilter | More] ->
            ?SLOG(warning, #{
                msg => "subscribe_hook_returned_multiple_topic_filters",
                picked => NTopicFilter,
                ignored => More
            }),
            {NTopicFilter, pending};
        [] ->
            %% TODO: Unclear what we are supposed to do here.
            {TopicFilter, ?RC_UNSPECIFIED_ERROR}
    end;
do_run_sub_hooks(_HookArgs, TFChecked = {_TopicFilter, _RC}) ->
    TFChecked.
...
enrich_subscribe(?SUBSCRIBE_PACKET(_PacketId, Properties, TopicFilters), Channel) ->
