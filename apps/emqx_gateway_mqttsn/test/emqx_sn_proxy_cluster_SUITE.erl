...
t_asleep_pingreq_does_not_resume_across_nodes_before_connect(Config) ->
    QoS = 1,
    SleepDuration = 5,
    ...
    try
        ...
        send_connect_msg(Socket2, Port2, ClientId, CleanSession),
        ...
        ?retry(
            50,
            20,
            #{conn_state := connected} = erpc:call(Node2, emqx_gateway_cm, get_chan_info, [
                mqttsn, ClientId
            ])
        ),
