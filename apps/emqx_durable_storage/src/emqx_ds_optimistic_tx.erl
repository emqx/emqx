...
-spec start_link(gen_server:server_name(), emqx_ds:db(), emqx_ds:shard(), module()) -> {ok, pid()}.
start_link(Name, DB, Shard, CBM) ->
    gen_statem:start_link(Name, ?MODULE, [DB, Shard, CBM], []).
...
new_kv_tx_ctx(CBM, DB, Shard, Generation0, Opts) ->
    ...
    maybe
        ...
        new_kv_tx_ctx(Shard, Generation, IsLatest, Serial, Leader, Opts)
    else
        _ ->
            ?err_rec(leader_down)
    end.
