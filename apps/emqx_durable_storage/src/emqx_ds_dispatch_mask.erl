...
%%
%% Therefore, encoding of 1111111 is [0], and encoding of 00001000 is
%% [4, 1].
-module(emqx_ds_dispatch_mask).

%% API:
-export([dec_make/1, dec_pop/1, filter/2]).
...
enc_push_true(Index, enc_s_empty) ->
    ...
    #enc_s{last_true_pos = Index, n_trues = 1, flips = [Index]};
...
enc_finalize(_, enc_s_empty) ->
    [];
enc_finalize(Size, #enc_s{last_true_pos = LastIndex, flips = Flips}) when
    LastIndex =:= Size - 1
->
...
enc_finalize(Size, #enc_s{last_true_pos = LastTrue, flips = Flips, n_trues = N}) when
    Size > LastTrue + 1
->
    %% Last element(s) of the vector were 0s, add one last flip:
...
filter(L, Encoding) ->
    do_filter(L, dec_make(Encoding), []).

%%================================================================================
%% Internal functions
%%================================================================================

-spec do_encode(non_neg_integer(), [boolean()], enc()) -> enc().
do_encode(N, [], Acc) ->
    enc_finalize(N, Acc);
do_encode(N, [false | Rest], Acc) ->
    do_encode(N + 1, Rest, Acc);
do_encode(N, [true | Rest], Acc0) ->
    Acc = enc_push_true(N, Acc0),
    do_encode(N + 1, Rest, Acc).

-spec do_filter([A], dec(), [A]) -> [A].
do_filter([], _, Acc) ->
    lists:reverse(Acc);
do_filter([Elem | Rest], Dec0, Acc) ->
    {Keep, Dec} = dec_pop(Dec0),
    case Keep of
        true ->
            do_filter(Rest, Dec, [Elem | Acc]);
        false ->
            do_filter(Rest, Dec, Acc)
    end.
