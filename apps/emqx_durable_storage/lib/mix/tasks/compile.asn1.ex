defmodule Mix.Tasks.Compile.Asn1 do
  ...
  def run(_args) do
    ...
    asn1_opts = config[:asn1_opts] || %{}
    out_dir = Map.fetch!(asn1_opts, :out_dir)
    opts = [:per, :noobj, outdir: to_charlist(out_dir)]

    manifest_data = read_manifest(manifest())
    manifest_modified_time = Mix.Utils.last_modified(manifest())
    Enum.each(asn1_srcs, &compile(&1, opts, app_root, manifest_modified_time))
    write_manifest(manifest(), manifest_data)

    {:noop, []}
  end

  defp compile(src_path, opts, app_root, manifest_modified_time) do
    src_path =
      app_root
      |> Path.join(src_path)
      |> Path.expand()
    if stale?(src_path, manifest_modified_time) do
      ...
      :ok = :asn1ct.compile(to_charlist(src_path), opts)
    else
      Mix.shell().info("file up to date, not compiling: #{src_path}")
    end
  end
