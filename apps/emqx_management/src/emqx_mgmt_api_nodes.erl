...
restart_listeners_api() ->
    Metadata = #{
        put => #{
            ...
            responses => #{
                <<"404">> => emqx_mgmt_util:not_found_schema(
                    <<"Node name error or plugin name not found">>,
                    ["ERROR_NODE", "PLUGIN_NAME_NOT_FOUND"]),
                <<"200">> => #{
...
list_listeners(#{node := Node}) ->
    case emqx_mgmt:list_listeners(Node) of
        ...
        Listeners ->
            {200, emqx_json:encode(listeners_info(Listeners))}
    end.
