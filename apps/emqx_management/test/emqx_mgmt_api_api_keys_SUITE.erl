...
t_create_optional_fields(_Config) ->
    ...
    %% (the Dashboard *user* field name), which is not part of the API key
    %% schema and is silently ignored by request validation. It must not 500;
    %% the key is created with an empty note.
