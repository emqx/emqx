    ...
    IncorrectConfig =
        CorrectConfig#{
            query => <<"SELECT password_hash, salt, is_superuser_str as is_superuser
                          FROM wrong_table where username = ${mqtt-username} LIMIT 1">>},
...
test_is_superuser({Field, Value, ExpectedValue}) ->
    ...
    "FROM users where username = ${mqtt-username} LIMIT 1",
     ...
     #{data => #{
       ...
       config_params => #{
              query => <<"SELECT password_hash, salt, is_superuser_int as is_superuser
                            FROM users where username = ${mqtt-clientid} LIMIT 1">>,
