...
%% not protected by anything any more: a new package may take it over.  What is
%% left of its path is still the layout of a plugin installation, which is the
%% only provenance such a leftover can have.
preflight_allows_module_left_from_removed_plugin_test() ->
    with_plugin_env(fun() ->
        Mod = removed_plugin_leftover_mod,
        RemovedBase = filename:absname("emqx_plugins_apps_tests_removed_install"),
        try
            LibDir = lib_dir(?PLUGIN),
            RemovedNameVsn = "removedplug-1.0.0",
            RemovedPluginDir = filename:join(RemovedBase, RemovedNameVsn),
            RemovedEbin = filename:join([RemovedPluginDir, RemovedNameVsn, "ebin"]),
            ok = filelib:ensure_dir(filename:join(RemovedEbin, "dummy")),
            ok = file:write_file(filename:join(RemovedPluginDir, "release.json"), <<"{}">>),
            Bin = compile_beam(Mod, plain_src(Mod, original)),
            RemovedBeam = beam_path(RemovedEbin, Mod),
            ok = file:write_file(RemovedBeam, Bin),
            {module, Mod} = code:load_binary(Mod, RemovedBeam, Bin),
            ok = file:del_dir_r(RemovedBase),
            _Ebin = write_plugin_app(LibDir, ?PLUGIN, [{Mod, Bin}]),
            ?assertEqual(ok, emqx_plugins_apps:validate(#{rel_apps => [?PLUGIN]}, LibDir))
        after
            drop_module(Mod),
            _ = file:del_dir_r(RemovedBase)
        end
    end).

%% ... but a release module stays protected when its beam file is gone: an
%% upgrade replaces the release's `lib' directories, so a loaded module can
%% point at a file which no longer exists without being a plugin leftover.
preflight_rejects_release_module_whose_file_is_gone_test() ->
    with_plugin_env(fun() ->
        Mod = removed_release_mod,
        ReleaseBase = filename:absname("emqx_plugins_apps_tests_removed_release"),
        try
            LibDir = lib_dir(?PLUGIN),
            ReleaseEbin = filename:join([ReleaseBase, "lib", "somerelease-1.0.0", "ebin"]),
            ok = filelib:ensure_dir(filename:join(ReleaseEbin, "dummy")),
            OldBin = compile_beam(Mod, plain_src(Mod, release)),
            OldBeam = beam_path(ReleaseEbin, Mod),
            ok = file:write_file(OldBeam, OldBin),
            {module, Mod} = code:load_binary(Mod, OldBeam, OldBin),
            ok = file:del_dir_r(ReleaseBase),
            NewBin = compile_beam(Mod, plain_src(Mod, plugin)),
            _Ebin = write_plugin_app(LibDir, ?PLUGIN, [{Mod, NewBin}]),
            ?assertMatch(
                {error, #{
                    msg := "plugin_beam_load_conflict",
                    conflict := loaded_outside_plugins
        ...
        after
            drop_module(Mod),
            _ = file:del_dir_r(ReleaseBase)
        end
    end).

%% A module loaded out of an archive is loaded from a path below the archive
%% file (`<bundle>/<name>-<vsn>.ez/<app>/ebin'), which is not the ebin directory
%% of a plugin installation, and it has no file of its own to read -- not even
%% once the archive has been removed.
preflight_rejects_module_loaded_from_archive_test() ->
    with_plugin_env(fun() ->
        Mod = archived_mod,
        ArchiveBase = filename:absname("emqx_plugins_apps_tests_archive"),
        try
            LibDir = lib_dir(?PLUGIN),
            Archive = filename:join(ArchiveBase, "archivesomething-1.0.ez"),
            ok = filelib:ensure_dir(Archive),
            ok = file:write_file(Archive, <<"archive">>),
            Beam = filename:join([
                Archive, "archivesomething-1.0", "ebin", atom_to_list(Mod) ++ ".beam"
            ]),
            Bin = compile_beam(Mod, plain_src(Mod, archive)),
        ...
        after
            drop_module(Mod),
            _ = file:del_dir_r(ArchiveBase)
        end
    end).

%% A module can be loaded from a bare filename which is not a path to a file at
%% all (`emqx_schema_registry_serde' loads generated code that way): it has no
%% plugin provenance, wherever the node's working directory happens to be.
preflight_rejects_module_loaded_from_bare_filename_test() ->
    with_plugin_env(fun() ->
        Mod = bare_filename_mod,
        try
            LibDir = lib_dir(?PLUGIN),
            Bin = compile_beam(Mod, plain_src(Mod, generated)),
            {module, Mod} = code:load_binary(Mod, atom_to_list(Mod) ++ ".memory", Bin),
            ?assertEqual(atom_to_list(Mod) ++ ".memory", code:which(Mod)),
            NewBin = compile_beam(Mod, plain_src(Mod, plugin)),
            _Ebin = write_plugin_app(LibDir, ?PLUGIN, [{Mod, NewBin}]),
            ?assertMatch(
                {error, #{
...
%% Plugin lifecycle operations are not serialized: another load can publish a
%% module from another file after this transaction did, and before the
%% transaction is rolled back.  The rollback must then leave that code alone
%% instead of dropping or overwriting it.
%%
%% A concurrent load of the very same file can not be told apart by the code
%% server; that residual collision is documented in `emqx_plugins_apps'.
rollback_leaves_module_published_by_another_load_test() ->
    with_plugin_env(fun() ->
        Mod = other_load_mod,
        %% Both outside the install directory, so that they are cleaned up here
        %% rather than by `with_plugin_env/1'.
        OursBase = filename:absname("emqx_plugins_apps_tests_tx_ours"),
        OtherBase = filename:absname("emqx_plugins_apps_tests_tx_other"),
        try
            OursEbin = filename:join(OursBase, "ebin"),
            OtherEbin = filename:join(OtherBase, "ebin"),
            ok = filelib:ensure_dir(filename:join(OursEbin, "dummy")),
            ok = filelib:ensure_dir(filename:join(OtherEbin, "dummy")),
            OursBin = compile_beam(Mod, plain_src(Mod, ours)),
            OursBeam = beam_path(OursEbin, Mod),
            ok = file:write_file(OursBeam, OursBin),
            OtherBin = compile_beam(Mod, plain_src(Mod, other)),
            OtherBeam = beam_path(OtherEbin, Mod),
            ok = file:write_file(OtherBeam, OtherBin),
            FailBin = compile_beam(tx_z_fail, failing_onload_src(tx_z_fail)),
            FailBeam = beam_path(OursEbin, tx_z_fail),
            ok = file:write_file(FailBeam, FailBin),
            Plan = [
                #{
                    app => tx_plugin_app,
                    ebin => OursEbin,
                    spec => {application, tx_plugin_app, [{vsn, "1.0.0"}]},
                    entries => [
                        #{module => Mod, beam => OursBeam, bin => OursBin},
                        #{module => tx_z_fail, beam => FailBeam, bin => FailBin}
                    ]
                }
            ],
            Tx0 = #{
            ...
            {error, #{msg := "failed_to_load_plugin_beam"}, Tx} =
                emqx_plugins_apps:do_load(Plan, Tx0),
            ?assertEqual(filename:absname(OursBeam), filename:absname(code:which(Mod))),
            %% Another load takes the module over while this transaction unwinds.
            {module, Mod} = code:load_binary(Mod, OtherBeam, OtherBin),
            ?assertEqual([], emqx_plugins_apps:rollback(Tx)),
            ?assertEqual(other, Mod:ping()),
            ?assertEqual(filename:absname(OtherBeam), filename:absname(code:which(Mod)))
        after
            drop_module(Mod),
            _ = file:del_dir_r(OursBase),
            _ = file:del_dir_r(OtherBase)
        end
    end).
...
%% The shared application exception must also hold for an installation made
%% under another install directory, which `plugin_name_of/1' recognises by its
%% manifest: the install directory is configurable, and the shared application
%% loaded from the other one has exactly the same spec.
preflight_allows_shared_app_module_from_other_install_dir_test() ->
    with_plugin_env(fun() ->
        Mod = shared_other_dir_mod,
        OtherBase = filename:absname("emqx_plugins_apps_tests_shared_other_install"),
        try
            OtherNameVsn = "otherdirsharedplug-1.0.0",
            NewNameVsn = "newdirsharedplug-1.0.0",
            AppNameVsn = "sharedfromotherdir-1.0.0",
            OtherPluginDir = filename:join(OtherBase, OtherNameVsn),
            OtherEbin = filename:join([OtherPluginDir, AppNameVsn, "ebin"]),
            ok = filelib:ensure_dir(filename:join(OtherEbin, "dummy")),
            ok = file:write_file(filename:join(OtherPluginDir, "release.json"), <<"{}">>),
            {AppName, AppVsn} = emqx_plugins_utils:parse_name_vsn(AppNameVsn),
            ok = file:write_file(
                filename:join(OtherEbin, atom_to_list(AppName) ++ ".app"),
                app_file(AppName, AppVsn, [Mod])
            ),
            OtherBin = compile_beam(Mod, plain_src(Mod, shared_v1)),
            OtherBeam = beam_path(OtherEbin, Mod),
            ...
            NewLibDir = lib_dir(NewNameVsn),
            NewBin = compile_beam(Mod, plain_src(Mod, shared_v2)),
            NewEbin = write_plugin_app(NewLibDir, AppNameVsn, [{Mod, NewBin}]),
            ?assertEqual(
                ok,
                emqx_plugins_apps:load(#{rel_apps => [AppNameVsn]}, NewLibDir)
            ),
            ?assertEqual(beam_path(NewEbin, Mod), code:which(Mod))
        after
            drop_module(Mod),
