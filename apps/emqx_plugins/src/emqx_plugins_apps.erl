...
%% module, where its previous code came from, whether that is the very file this
%% package published (then its bytes are not the previous code any more), and
%% the file this transaction published it from.  The rollback only touches the
%% module while the code server still runs code loaded from that file, so that a
%% concurrent load from another file is not rolled back (a concurrent load from
%% the same file can not be told apart: see `restore_module/1').
...
%% Whether the application loaded from `LoadedEbinDir' is the very same
%% application spec as the one this package ships, installed by another plugin.
%%
%% Any ebin directory with plugin provenance counts, not only the ones below the
%% configured install directory: the install directory is configurable, and a
%% plugin installed under another one (a previous configuration, a test work
%% directory) is still recognised by its manifest, exactly like
%% `plugin_name_of/1' recognises it for module ownership.
...
%% The file the code server holds for `Module', or `non_existing' when the module
%% is not loaded.  `code:which/1' alone can not be used: it also searches the
%% code path for a module which is not loaded, and phase 1 has already put the
%% package's own ebin there, so it would report the package beam as if it were
%% the code of a module which has none.
...
%%
%% In `publish/1' it tells whether the previous code was loaded from the very
%% file this package publishes.  It happens on an in-place reinstall, where the
%% package has already overwritten that file: its bytes are then the package's,
%% and the previous code can not be read back from it.
%%
...
%%
%% On rollback the same comparison, against the file this transaction published,
%% tells its own code apart from code another load published in the meantime.
...
%% Put back the code which `publish/1' replaced, but only while the code server
%% still runs code loaded from the file this transaction published.  Plugin
%% lifecycle operations are not serialized: another load may have published the
%% module in the meantime, and its code must be left alone rather than dropped or
%% overwritten by this rollback.  `code:load_binary/3' records the filename it is
%% given as the module's file, which is what this checks against.
%%
%% This is best effort: the code server keeps no per-load identity, so neither a
%% concurrent load of the same module from the same file, nor a load that landed
%% between `publish/1' reading the previous code and loading this one, can be
%% told apart from this transaction.  Closing that requires serializing the
%% whole preflight/publish/rollback sequence, which stays the callers' premise
...
%% The code server no longer runs the code this transaction published for the
%% module: another load took it over from another file, or it is not loaded any
%% more.  Either way this rollback has nothing of its own to put right.
...
%% The beam file of a loaded module, which is not below a plugin installation
%% directory, is gone.
%%
%% What is left of the path of a plugin's own beam file is always
%% `<plugin-dir>/<app-dir>/ebin/<module>.beam': `ebin' is the last directory,
%% and above it is the plugin directory, a real directory named `<name>-<vsn>'.
%% Only such a path is accepted as the leftover of a plugin which no longer
%% exists on disk (its directory was purged, or it was installed under an
%% install directory this configuration does not use any more).
%%
%% A release or OTP module installed under the release's `lib' directory does not
%% have such a path and stays protected even though its file is gone (an upgrade
%% replaces the release's `lib' directories): its ebin directory sits directly
%% below that `lib' directory, whose name is not `<name>-<vsn>'.  Nor is a module
%% loaded out of a `.ez' archive (whose path names the archive, not a plugin
%% directory, and stays that way after the archive is removed) or from a bare
%% filename given to `code:load_binary/3' -- both are loaded without a file of
%% their own to read.
check_removed_source(Module, File, AppNameVsn) ->
