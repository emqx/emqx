...
-doc """
Verifies that the configured `scope` is sent in the token endpoint request
body, and that it is omitted when not configured.  This is an OAuth2 wire
contract that must survive the migration of the token cache to
`emqx_connector_oauth2`.
""".
t_oauth_client_credentials_scope(TCConfig0) ->
    #{tc_config := TCConfig, port := Port} = setup_oauth_scenario(TCConfig0),
    NowS = erlang:system_time(second),
    JWT = generate_unsigned_jwt(#{
        <<"exp">> => NowS + 3600,
        <<"iat">> => NowS,
    ...
    RespBody = emqx_utils_json:encode(#{<<"access_token">> => JWT, <<"expires_in">> => 3600}),

    ct:pal("scope is sent in the token request body when configured"),
    set_capturing_handler(self(), RespBody),
    ?assertMatch(
        {201, #{<<"status">> := <<"connected">>}},
        create_connector_api(TCConfig, oauth_auth_override(Port, <<"my-scope">>))
    ),
    BodiesWithScope = receive_all_bodies(2_000),
    ?assertMatch([_ | _], BodiesWithScope),
    ?assert(bodies_with_scope(BodiesWithScope, <<"my-scope">>)),
    emqx_bridge_v2_testlib:delete_all_bridges_and_connectors(),
    emqx_connector_oauth2:clear_cache(),

    ct:pal("scope is omitted from the token request body when not configured"),
    set_capturing_handler(self(), RespBody),
    ?assertMatch(
        {201, #{<<"status">> := <<"connected">>}},
        create_connector_api(TCConfig, oauth_auth_override(Port, undefined))
    ),
    BodiesNoScope = receive_all_bodies(2_000),
    ?assertMatch([_ | _], BodiesNoScope),
    ?assert(bodies_without_scope(BodiesNoScope)),
    emqx_bridge_v2_testlib:delete_all_bridges_and_connectors(),
    emqx_connector_oauth2:clear_cache(),
...
-doc """
Verifies that the token endpoint is hit only once even though multiple messages
are published: the access token is cached and reused across requests, not
re-fetched per produce.  This caching behavior must survive the migration of the
token cache to `emqx_connector_oauth2`.
""".
t_oauth_client_credentials_token_cached(TCConfig0) ->
    #{tc_config := TCConfig, port := Port} = setup_oauth_scenario(TCConfig0),
    NowS = erlang:system_time(second),
    JWT = generate_unsigned_jwt(#{
        <<"exp">> => NowS + 3600,
    ...
    ?assertMatch(
        {201, #{<<"status">> := <<"connected">>}},
        create_connector_api(TCConfig, oauth_auth_override(Port, <<"scope1">>))
    ),
    ?assertMatch({201, _}, create_action_api(TCConfig, #{})),
    #{topic := RuleTopic} = simple_create_rule_api(TCConfig),
    C = start_client(),
    %% The connector startup fetched the token exactly once.
    ?assertEqual(1, length(receive_all_bodies(2_000))),
    %% Publish several messages; each flows through the connector, which must
    %% reuse the cached token rather than re-fetching it per request.
    lists:foreach(
        fun(N) ->
            emqtt:publish(C, RuleTopic, integer_to_binary(N), [{qos, 1}])
        end,
        lists:seq(1, 5)
    ),
    %% Allow the messages to flow through the rule action to Kafka, then assert
    %% no additional token fetch happened.
    ct:sleep(1_000),
