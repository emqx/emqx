...
-doc """
Exercises OAuth client-credentials authentication for the Kafka *consumer*
connector.  The consumer shares the `sasl/1' path with the producer but derives
its brod client id differently (an atom, via `make_client_id/1'), so this guards
the consumer-side token registration when the cache is migrated to
...
t_oauth_client_credentials_authn(TCConfig) ->
    ...
    Body = emqx_utils_json:encode(#{<<"access_token">> => JWT, <<"expires_in">> => 3600}),
