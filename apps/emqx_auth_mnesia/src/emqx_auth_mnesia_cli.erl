...
add_user(Login, Password) ->
    ...
    ret(ekka_mnesia:transaction(emqx_auth_mnesia_shard, fun insert_user/1, [User])).
...
update_user(Login, NewPassword) ->
    ret(ekka_mnesia:transaction(emqx_auth_mnesia_shard, fun do_update_user/2, [Login, encrypted_data(NewPassword)])).
...
remove_user(Login) ->
    ret(ekka_mnesia:transaction(emqx_auth_mnesia_shard, fun mnesia:delete/1, [{?TABLE, Login}])).
