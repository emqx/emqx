...
-spec on_session_takenover(emqx_exhook_pb:session_takenover_request())
    -> {ok, emqx_exhook_pb:empty_success(), grpc:metadata()}
     | {error, term()}.
on_session_takenover(Req) ->
    on_session_takenover(Req, #{}, #{}).

-spec on_session_takenover(emqx_exhook_pb:session_takenover_request(), grpc:options())
    -> {ok, emqx_exhook_pb:empty_success(), grpc:metadata()}
     | {error, term()}.
on_session_takenover(Req, Options) ->
    on_session_takenover(Req, #{}, Options).

-spec on_session_takenover(emqx_exhook_pb:session_takenover_request(), grpc:metadata(), grpc_client:options())
    -> {ok, emqx_exhook_pb:empty_success(), grpc:metadata()}
     | {error, term()}.
on_session_takenover(Req, Metadata, Options) ->
    grpc_client:unary(?DEF(<<"/emqx.exhook.v1.HookProvider/OnSessionTakenover">>,
                           session_takenover_request, empty_success, <<"emqx.exhook.v1.SessionTakenoverRequest">>),
                      Req, Metadata, Options).
