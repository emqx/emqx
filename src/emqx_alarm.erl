...
get_alarms(activated) ->
    ge_server:call(?MODULE, {get_alarms, activated});
...
normalize_message(system_memory_high_watermark, _Details) ->
    io_lib:format("System memory usage is higher than ~p%", [emqx_os_mon:get_sysmem_high_watermark()]);
normalize_message(process_memory_high_watermark, _Details) ->
    io_lib:format("Process memory usage is higher than ~p%", [emqx_os_mon:get_procmem_high_watermark()]);
normalize_message(high_cpu_usage, #{usage := Usage}) ->
    io_lib:format("~p% cpu usage", [Usage]);
normalize_message(too_many_processes, #{high_watermark := HightWatermark}) ->
    io_lib:format("High_watermark: ~p%", [HightWatermark]).
