...
t_trace_topic(_Config) ->
    ...
    %% Get current traces
    ?assertEqual([#{type => topic, topic => <<"x/#">>, name => <<"x/#">>, level => debug, dst => "tmp/topic_trace_x.log"},
        #{type => topic, topic => <<"y/#">>, name => <<"y/#">>, level => debug, dst => "tmp/topic_trace_y.log"}],
        emqx_tracer:lookup_traces()),
...
t_is_match(_Config) ->
    ClientId = <<"test">>,
    ?assert(emqx_tracer:is_match(#{clientid => ClientId, type => clientid, labels => []}, #{clientid => ClientId}, warning)),
    ?assert(emqx_tracer:is_match(#{clientid => ClientId, type => clientid, labels => []}, #{clientid => ClientId}, debug)),
    ?assertNot(emqx_tracer:is_match(#{clientid => ClientId, type => clientid, labels => ['PUBLISH']}, #{clientid => ClientId}, debug)),
    ?assert(emqx_tracer:is_match(#{clientid => ClientId, type => clientid, labels => []}, #{clientid => ClientId, trace_label => 'PUBLISH'}, debug)),
    ?assert(emqx_tracer:is_match(#{clientid => ClientId, type => clientid, labels => ['PUBLISH']}, #{clientid => ClientId, trace_label => 'PUBLISH'}, debug)),
    ?assertNot(emqx_tracer:is_match(#{clientid => ClientId, type => clientid, labels => ['SUBACK']}, #{clientid => ClientId, trace_label => 'PUBLISH'}, debug)),
    ?assert(emqx_tracer:is_match(#{clientid => ClientId, type => clientid, labels => ['SUBACK']}, #{clientid => ClientId, trace_label => 'ALL'}, debug)),
    ?assertNot(emqx_tracer:is_match(#{clientid => ClientId, type => clientid, labels => []}, #{clientid => <<"Bad">>}, warning)),
    ?assertNot(emqx_tracer:is_match(#{clientid => ClientId, type => clientid, labels => []}, #{clientid => <<"Bad">>, trace_label => 'PUBLISH'}, debug)),

    Topic = <<"/test/#">>,
    ?assert(emqx_tracer:is_match(#{type => topic, topic => Topic, labels => []}, #{topic => <<"/test/1">>}, warning)),
    ?assert(emqx_tracer:is_match(#{type => topic, topic => Topic, labels => []}, #{topic => <<"/test/1/2">>}, debug)),
    ?assertNot(emqx_tracer:is_match(#{type => topic, topic => Topic, labels => ['SUBSCRIBE']}, #{topic => <<"/test/1/2">>}, debug)),
    ?assert(emqx_tracer:is_match(#{type => topic, topic => Topic, labels => []}, #{topic => <<"/test/3">>, trace_label => 'PUBLISH'}, debug)),
    ?assert(emqx_tracer:is_match(#{type => topic, topic => Topic, labels => ['PUBLISH']}, #{topic => <<"/test/398/">>, trace_label => 'PUBLISH'}, debug)),
    ?assertNot(emqx_tracer:is_match(#{type => topic, topic => Topic, labels => ['SUBACK']}, #{topic => <<"/test/1/xy/y">>, trace_label => 'PUBLISH'}, debug)),

    ?assertNot(emqx_tracer:is_match(#{type => topic, topic => Topic, labels => ['PUBLISH']}, #{topic => <<"/t1est/398/">>, trace_label => 'PUBLISH'}, debug)),
    ?assertNot(emqx_tracer:is_match(#{type => topic, topic => Topic, labels => []}, #{topic => <<"/t1est/1/xy/y">>, trace_label => 'PUBLISH'}, debug)),
    ok.
