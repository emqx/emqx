...
start_traces(_Config) ->
    ...
    ok = emqx_tracer:start_trace({client_id, <<"client">>}, all, "log/clientid_trace.log"),
    ok = emqx_tracer:start_trace({topic, <<"topic">>}, all, "log/clientid_trace.log"),
    Result = emqx_tracer:lookup_traces(),
    ?assertEqual([{{client_id,<<"client">>},{all,"log/clientid_trace.log"}},{{topic,<<"topic">>},{all,"log/clientid_trace.log"}}], Result),
