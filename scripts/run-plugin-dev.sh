...
## Build a plugin, install it into the running EMQX dev release, then
## enable and start it. Optionally attach a remote console.
...
Example:
  scripts/run-plugin-dev.sh emqx_offline_messages --attach

What it does:
1) Build plugin package with: make plugin-<plugin_app>
2) Start EMQX release node if not already running
3) Copy plugin .tar.gz to the release plugin install_dir
4) Run:
