...
#
# <extra-dns-sans> is space separated and defaults to the node names used by
# start-two-nodes-in-docker.sh. The server certificate has to carry them: EMQX
# 5.4 runs OTP 25, whose inet_tls_dist:do_setup_connect/8 prepends
# {server_name_indication, Address} to the client options (OTP 25
# ssl/inet_tls_dist.erl:588), so an option file cannot disable the host name
# check there and the check must pass instead.
#
...
EXTRA_DNS_SANS="$EXTRA_DNS_SANS" ./scripts/gen-test-certs.sh "$DIR"
