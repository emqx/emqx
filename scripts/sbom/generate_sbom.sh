...
#
# This script:
# 1. Runs mix sbom to generate CycloneDX SBOM
# 2. Converts CycloneDX to SPDX format
...
#   --skip-mix         Skip running mix sbom (use existing bom.cdx.json)
...
Output files:
  - bom.cdx.json              CycloneDX SBOM (generated by mix sbom)
...
FILTER_SCRIPT="${SCRIPT_DIR}/filter_sbom.py"
...
if [[ "${SKIP_MIX}" == "false" ]]; then
    if [[ -f "${CDX_FILE}" && "${FORCE}" == "false" ]]; then
        ...
        echo "Step 1/3: Generating CycloneDX SBOM..." >&2
        mix sbom --output "${CDX_FILE}" --force
        echo "✓ CycloneDX SBOM generated: ${CDX_FILE}" >&2
    fi
else
    echo "Step 1/3: Skipping mix sbom (using existing file)" >&2
fi
...
if [[ "${SKIP_CONVERT}" == "false" ]]; then
    if [[ -f "${SPDX_FILE}" && "${FORCE}" == "false" ]]; then
        ...
        echo "Step 2/3: Converting CycloneDX to SPDX..." >&2
        python3 "${CONVERTER_SCRIPT}" "${CDX_FILE}" -o "${SPDX_FILE}"
        echo "✓ SPDX SBOM converted: ${SPDX_FILE}" >&2
    fi
else
    echo "Step 2/3: Skipping conversion (using existing file)" >&2
fi
...
if [[ "${SKIP_FILTER}" == "false" ]]; then
    if [[ -f "${FILTERED_FILE}" && "${FORCE}" == "false" ]]; then
        ...
        echo "Step 3/3: Filtering SPDX SBOM..." >&2
    ...
    echo "Step 3/3: Skipping filtering (using existing file)" >&2
fi
