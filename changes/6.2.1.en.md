...
Breaking changes in a bug fix release due to security enhancement
...
- [#17099](https://github.com/emqx/emqx/pull/17099) Fix routing table inconsistency when a disconnected core node reconnects.
...
- [#17507](https://github.com/emqx/emqx/pull/17507) [#17535](https://github.com/emqx/emqx/pull/17535) Fixed several gateway paths that could reach publish or subscribe handling before authentication completed.
  ...
  CoAP connectionless `/ps` publish and observe requests now authenticate before entering pub/sub handling. Under the hardened profile, such requests are rejected when no authentication is configured unless listener authentication is explicitly disabled. Connectionless observe requests also now skip the `session.subscribed` hook, avoiding a `{badmatch, undefined}` crash when the hook callback tried to look up channel info that the connectionless path never establishes.
