...
EMQX 6.1.0 introduces Message Streams, enhanced namespace capabilities, and centralized certificate management.

**Message Streams**

Message Streams provide durable collections of messages identified by a topic filter, with explicit lifecycle management. Messages matching a stream's topic filter are automatically appended, enabling consumption with ordering guarantees and support for multiple consumers. Clients can subscribe to streams using the special topic format `$s/<timestamp>/topic/filter` to consume messages from a specific point in time.
...
- [#16154](https://github.com/emqx/emqx/pull/16154) Added support for referencing managed certificate files in SSL options of listeners and client.
...
- [#16368](https://github.com/emqx/emqx/pull/16368) Upgrades the underlying runtime system from Erlang/OTP 27 to Erlang/OTP 28.

- [#16377](https://github.com/emqx/emqx/pull/16377) Reduced the number of pre-allocated metrics counters, which should contribute to reduced memory usage, specially in clusters using lots of namespaces.
...
- [#16397](https://github.com/emqx/emqx/pull/16397) Add TLS certificate validation before listener start. Fail-fast if listener is misconfigured with invalid certificates.
...
- [#16368](https://github.com/emqx/emqx/pull/16368) The internal regex engine has been upgraded to PCRE2.
