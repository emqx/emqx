...
- [#16489](https://github.com/emqx/emqx/pull/16489) Fix error that the following rule functions always return 'undefined'
  ...
  Note: This is a backward compatibility fix for EMQX v4, the functions are not ducmented in EMQX v5 and later, the encouraged usage is name reference directly to rule evaluation context. For example, `SELECT clientid ..` instead of `SELECT clientid()`.
