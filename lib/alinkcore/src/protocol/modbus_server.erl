...
handle_info({help_run_tasks, DeviceInfo, TaskThings}, TcpState) ->
    #device_info{addr = Addr, product_id = ProductId} = DeviceInfo,
    case run_task(DeviceInfo, TaskThings, TcpState, #{<<"ts">> => os:system_time(second)}) of
        {error, timeout} ->
            {noreply, TcpState};
        {error, Reason} ->
            {stop, Reason, TcpState};
...
%%作为网关进程调用
handle_info({task, Time}, #tcp_state{ state =#state{ tasks = Tasks} } = TcpState) ->
    #state{
        addr = Addr,
        productId = ProductId
    } = TcpState#tcp_state.state,
    erlang:send_after(Time * 1000, self(), {task, Time}),
    Things = maps:get(Time, Tasks),
    DeviceInfo =
        #device_info{
            addr = Addr,
            product_id = ProductId,
    ...
    case run_task(DeviceInfo, Things, TcpState, #{<<"ts">> => os:system_time(second)}) of
        {error, timeout} ->
            {noreply, TcpState};
        {error, Reason} ->
            {stop, Reason, TcpState};
...
%%作为子设备进程调用
handle_info({task, Time}, #sub_device_state{ tasks = Tasks,
                                                        sub_addr = SubDevAddr,
                                                        addr = Addr,
                                                        product_id = ProductId,
                                                        gateway = GatewayPid} = State) ->
    erlang:send_after(Time * 1000, self(), {task, Time}),
    TaskThings = maps:get(Time, Tasks),
...
run_task(DeviceInfo, [Thing | Things], TcpState, Acc) ->
    case send_to_device(DeviceInfo, Thing#{ <<"type">> => <<"read">> }, TcpState) of
        ...
        {error, timeout} ->
            logger:error("run task for ~p thing ~p failed:~p", [DeviceInfo, Thing, timeout]),
            Name = maps:get(<<"name">>, Thing),
            NAcc = Acc#{ Name => #{
                            <<"value">> => <<"alink_ignore_sync_data_timeout">>,
                            <<"ts">> => os:system_time(second)
                        }},
            run_task(DeviceInfo, Things, TcpState, NAcc);
        {error, Reason} ->
            logger:error("run task for ~p thing ~p failed:~p", [DeviceInfo, Thing, Reason]),
            run_task(DeviceInfo, Things, TcpState, Acc)
    end.
...
send_to_device(DeviceInfo, Thing, #tcp_state{ip = Ip} = TcpState) ->
    ...
    case alinkcore_tcp:sync_send(TcpState, Payload, 10000, FilterFun) of
        {ok, Data} ->
            ...
            catch case decode_frame(SubDevAddr, Data, Thing) of
                      {'EXIT', Reason} ->
                          {error, Reason};
                      Other ->
                          Other
...
decode_frame(SubDevAddr, RData, Thing) ->
    decode_frame_1(SubDevAddr, RData, Thing).

decode_frame_1(SubDevAddr, <<RDataSlaveId:8, Data/binary>> = RData, Thing) when byte_size(Data) >= 7->
    SlaveId =
    ...
    case binary_to_integer(SlaveId, 16) of
        RDataSlaveId ->
            do_decode_frame(SubDevAddr, RData, Thing);
        _  ->
            decode_frame_1(SubDevAddr, Data, Thing)
    end;
decode_frame_1(SubDevAddr, Data, Thing) ->
    do_decode_frame(SubDevAddr, Data, Thing).

do_decode_frame(_SubDevAddr, <<>>, _Thing) ->
    {error, crc_error};
do_decode_frame(SubDevAddr, <<SlaveId:8, _/binary>> = Bin, Thing) ->
...
create_task_by_thing(#{<<"name">> := Name, <<"type">> := Type, <<"modbus">> := Opts} = Thing, Acc) ->
    case maps:get(<<"access">>, Thing, <<"read">>) of
        <<"write">> ->
            Acc;
        _ ->
            Freq = case maps:get(<<"freq">>, Thing, 0) of
                       0 -> 30;
                       V -> V
                   end,
