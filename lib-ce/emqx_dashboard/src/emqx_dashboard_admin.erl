...
add_user(Username, Password, Tags) when is_binary(Username), is_binary(Password) ->
    ...
    return(ekka_mnesia:transaction(emqx_management_shard, fun add_user_/1, [Admin])).
...
remove_user(Username) when is_binary(Username) ->
    ...
    return(ekka_mnesia:transaction(emqx_management_shard, Trans)).

-spec(update_user(binary(), binary()) -> ok | {error, term()}).
update_user(Username, Tags) when is_binary(Username) ->
    return(ekka_mnesia:transaction(emqx_management_shard, fun update_user_/2, [Username, Tags])).
