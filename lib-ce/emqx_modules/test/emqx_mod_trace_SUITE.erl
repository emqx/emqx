...
t_client_event(_Config) ->
    ...
    ct:sleep(400),
    {ok, Bin} = file:read_file(emqx_mod_trace:log_file(<<"test_client_id">>, Now)),
    ...
    ct:sleep(300),
    {ok, Bin2} = file:read_file(emqx_mod_trace:log_file(<<"test_client_id">>, Now)),
    ?assertEqual(true, erlang:byte_size(Bin) < erlang:byte_size(Bin2)),
    {ok, Bin3} = file:read_file(emqx_mod_trace:log_file(<<"test_topic">>, Now)),
    ?assertNotEqual(0, erlang:byte_size(Bin3)),
    ok = emqx_mod_trace:unload(test),
    ok.
